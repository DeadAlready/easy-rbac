...
RBAC.main = function easyRBACMiddle(opts) {
  const rbac = RBAC.create(opts);
  return function easyRBACMiddleware(req, res, next) {
    req.rbac = rbac;
    next();
  }
};

RBAC.can = function can(role, operation, params, errorCallback) {
  if(typeof operation !== 'string') {
    throw new TypeError('Expected second parameter to be string');
  }
  return function canAccess(req, res, next) {
    let $roles = typeof role === 'function' ? role(req, res) : Promise.resolve(role);
    let $params = typeof params === 'function' ? params(req, res) : Promise.resolve(params);

    Promise.all([$roles, $params]).then(([roles, data]) => req.rbac.can(roles, operation, data))
      .then(accessGranted => {
        if(!accessGranted) {
